...
namespace FeedRefresher.Playlists
{
...
Version=2";    
        ...
        private static void CreateDefaultPlaylist(string playlistPath)
        {
            var defaultContent = 
@"[playlist]
File1=C:\MP3s\music\station\radio chime.mp3
Title1=radio chime
Length1=5
NumberOfEntries=1
Version=2";
        ...
        public static void WriteNewPlaylist(string playlist, List<FRPodcastFileToProcess> mp3sForPlaylist)
        {
        ...
        public static void WriteNewPlaylist(string pathToPlaylist, List<FRPodcastFileToProcess> mp3sForPlaylist, int maxFilesInnewpodcastsPlaylist)
        {
            var existingPlaylist = GetPlaylist(pathToPlaylist);

            Log.Debug(string.Format("Started writing new playlist to: '{0}'", pathToPlaylist));
            
            var newPlaylist = IsnewpodcastsPlaylist(pathToPlaylist) ?
                                   CreatenewpodcastsPlaylist(mp3sForPlaylist, existingPlaylist, maxFilesInnewpodcastsPlaylist) :
        ...
        private static List<PlaylistFile> CreatenewpodcastsPlaylist(
            List<FRPodcastFileToProcess> mp3sForPlaylist, 
            List<PlaylistFile> existingPlaylist,
            int maxFilesInnewpodcastsPlaylist)
        {
            ...
            var mostRecentFiles = possiblePlaylistFiles.DistinctBy(file => file.File)
                                                       ...
                                                       .Take(maxFilesInnewpodcastsPlaylist);
        ...
        private static List<DatedPlaylistFile> GetNewPlaylistWithDates(List<FRPodcastFileToProcess> mp3sForPlaylist)
        {
        ...
        private static List<PlaylistFile> CreateStandardPlaylist(List<FRPodcastFileToProcess> mp3sForPlaylist, List<PlaylistFile> existingPlaylist)
        {
            var newPlaylist = new List<PlaylistFile>();
        ...
        private static PlaylistFile GetPlaylistFileFromProdcastFile(FRPodcastFileToProcess newFile)
        {
            var playlistFile = new PlaylistFile();
        ...
        private static bool IsnewpodcastsPlaylist(string pathToPlaylist)
        {
            return pathToPlaylist.ToLower().Contains(StringConstants.newpodcastsPlaylist.ToLower());
        }
    }
}
